...
1. **manual**：来自维修手册的内容，必须标注 documentId 和引用的原文关键内容
  ...
  "steps": [
    {
      ...
      "sources": [
        {"type": "manual", "documentId": "文档ID", "snippet": "引用的关键原文片段（不超过50字）"},
...
class MaintenanceAgent(BaseAgent):
    ...
    def _build_prompt(
        ...
        if retrieval_results:
            ...
            for i, doc in enumerate(retrieval_results):
                ...
                parts.append(f"\n### 手册片段{i+1}（documentId={doc_id}, 相关度={score:.2f}）")
                if doc_name:
    ...
    def _calc_confidence(
        ...
        # 收集检索到的 document_id 集合用于验证
        known_doc_ids = set()
        if retrieval_results:
            for doc in retrieval_results:
                if hasattr(doc, 'id'):
                    known_doc_ids.add(str(doc.id))
                elif isinstance(doc, dict):
                    did = doc.get('id') or doc.get('doc_id', '')
                    if did:
                        known_doc_ids.add(str(did))
